...
--EMPLOYEE테이블의 사원명, 이메일, 전화번호, 입사일, 급여조회
...
/*
    ...
    <컬럼명에 별칭 지정하기>
    산술연산을 하게되면 하게되면 컬럼명 이 지저분해진다. 이때 별칭을 부여해서 깔끔하게 가져올 수 있다.
    [표현법]
    컬럼명 별칭 / 컬럼명 AS 별칭 / 컬럼명 " 별칭" / 컬럼명 AS "별칭"
*/
SELECT EMP_NAME 사원명, SALARY AS 급여, BONUS "보너스" , (SALARY * 12) AS "연봉",((SALARY+(SALARY * BONUS))*12) AS "총 소득"
FROM EMPLOYEE;

/*
    <리터럴>
    임의로 지정한 문자열('')
    조회된 결과의 모든 행에 반복저으로 출력
...
/*
    ...
    [표현법]
    비교대상 걸럼 BETWEEN A AND B (A이상 B이하)
*/
...
/*
    ...
    2.'_' : 1글자를 대체하는 검색
     EX)비교할 대상 컬럼 LIKE '_문자 : 비교할대상컬럼값 문자앞에 아무글자나 한글자가 있는 값을 조회
        비교할 대상 컬럼 LIKE '문자_ : 비교할대상컬럼값 문자앞에 아무글자나 한글자가 있는 값을 조회
*/
...
--WHERE EMAIL LIKE '___%'; -> 와일드카드 문자때문에 정상출력이 되지않는데.
--와일드카드와 일반문자를 구분해줘야한다.
--데이터값으로 취급하고 싶은 와일드 카드 문자앞에 나만의 탈출문자를 제시해서 탈술시켜주면된다.
-------------------------------실습------------------------------
...
SELECT EMP_ID, EMP_NAME, SALARY,((SALARY+(SALARY * BONUS))*12) FROM EMPLOYEE WHERE BONUS IS NOT NULL AND EMP_NAME LIKE '%하%';