/*
    ...
    EX)
    회원탈퇴시 기존의 회원테이블에 데이터 DELETE한 수 곧바로 탈퇴한 회원들만 따로 보관하는 테이블에
    자동으로 INSERT 시켜야한다.
    ...
    *트리거의 종류
    -SQL문의 실행시기에 따른 분류
    -BEFORE TRIGGER : 내가 지정한 테이블 이벤트가 발생되기 전에 트리거 실행
    ...
    -행트리거 : 해당 SQL문 실행할 때마다 매번 트리거 실행
        ...
        > :OLD - BEFOR UPDATE(수정전 자료), BEFORE DELETE(삭제전 자료)
        > :NEW - AFTER INSERT(추가된 자료), AFTER UPDATE(수정후 자료)
        
    *트리거 생성 구문
    [표현식]
    CRAETE  [OR REPLACE] TRIGER 트리거명
    BEFORE | AFTER          INTSERT | UPDATE | DELETE    ON 테이블명
    [FOR EACH ROW]
    [DECLARE 변수선언]
    BEGIN
...
--상품번호 중복안되게끔 매번 새로운 번호를 발생시키는 시퀀스 생성
...
--TB_PRODUCT테이블에 매번 자동을 재고수량을 UPDATE하게 끔 트리거 작성
...
--200번 상품이 ㅣ오늘날짜로 100개 입고
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DECODE.NEXTVAL,200,SYSDATE,100,'입고');

INSERT INTO TB_PRODETAIL
VALUES(SEQ_DECODE.NEXTVAL,205,SYSDATE,100,'입고');