...
/*
    ...
    --NULLS LAST : 정렬하고자하나는 컬럼값에 NULL이 있을 경우 해당데이터 맨 마지막에 배치(ASC일때 기본값)
*/
...
--전 사원의 사원명, 연봉(보너스제외) 조회(이 때 연봉별 내림차순)
SELECT EMP_NAME, SALARY*12 AS "연봉"
FROM EMPLOYEE
--ORDER BY "연봉" DESC
ORDER BY 2 DESC; -- 순번가용가능 오라클은 전부 1부터 시작
...
/*
    ...
    -그룹함수 : N개의 값을 읽어들여서 1개의 값을 리턴(그룹을 지어서 그룹별로 함수실행 결과를 반환)
...
/*
    *INSTR
    문자열로부터 즉정 문자의 시작위치를 찾아서 반환
    
    INSTR(컬럼 | '문자열', '찾고자하는 문자', ['찾을 위치의 시작값, 순번']) -> 결과는 NUMBER
*/
SELECT INSTR('AABAACAABBAA','B') FROM DUAL; --앞쪽에 있는 첫 B는 3번재 위치에 있다.
...
/*
    *ROUND
    ...
    ROUND(NUMBER, [위치])
*/
SELECT ROUND(123.456) FROM DUAL; -- 기본자리수는 소수점 첫번째 자리에서 반올림
SELECT ROUND(123.456,2) FROM DUAL; -- 양수로 증가할 수록 소수점 뒤로 한칸씩 이동
...
-- *LAST_DAY(DATE) : 해당월을 ㅣ마지막 날짜 구해서 반환
...
SELECT TO_CHAR(2000000, 'L99999999') FROM DUAL; --현재 설정된 나라의 로컬 화폐단위 나타남
...
SELECT EMP_NAME,TO_CHAR(HIRE_DATE,'YYYY"년" MM"월" DD"일"') AS 입사일 FROM EMPLOYEE;

--년도와 관련된 포맷
SELECT TO_CHAR(SYSDATE,'YYYY'),
         TO_CHAR(SYSDATE,'YY'),
         TO_CHAR(SYSDATE,'RRRR') --RR룰이 따로 존재한다 -> 50년 이상값이 +100 
FROM DUAL;
...
-- 전 사원의 이름, 보너스폼함 연봉 조회
SELECT EMP_NAME, (SALARY + (SALARY *NVL(BONUS, 0)))*12
FROM EMPLOYEE;

-- * NVL2(컬럼, 반환값1, 반환값2)
--반환값1 : 해당 컬럼이 존재할 경우 보여줄 값
--반환값2 : 해당 컬럼이 NULL일 경우 보여줄 값
...
SELECT EMP_NAME, SALARY AS "인상전", DECODE(JOB_CODE,'J7',(SALARY *1.1),'J6',(SALARY *1.15),'J5',(SALARY *1.2),(SALARY *1.05)) AS 인상후
FROM EMPLOYEE;
...
SELECT EMP_NAME, SALARY, CASE WHEN SALARY >= 5000000 THEN '고급' WHEN SALARY >= 3500000 THEN '중급' ELSE '초급' END
FROM EMPLOYEE;
...
--COUNT(DISTINCT 컬럼) : 해당 컬럼값 중복을 제거한 수 행의 갯수를 반환
