...
<body>
	...
	*변수 선언과 동시에 초기화(c:set var="변수명" value="값" [scope="저장객체"])
	...
	<c:set var="num1" value="10" /> <%--<% pageContext.setAttribut("num1",10); --%>
	...
	*변수 삭제 (c:remove var="제거하고싶은 변수명" [scope=""])
	...
	request에서 삭제 후 : ${result}
	
	2) 모든 scope에서 삭제
	<c:remove var="result"/>
	모두 삭제 후 request : ${result}<br><br>
	...
	<pre>
	*변수(데이터) 출력 (c:out value="출력하고자하는 값" [default="기본값"] [excapeXml="true | false"])
	...
	향상된 for문 - (c:forEach var="변수명" items="순차적으로 접근하고자하는 객체(배열/컬렉션)" [varStatus="현재접근도니 요소의 상태값"])
	</pre>
	<%-- 
	...
	-구분자를 통해서 분리된 각 문자열에 순차적으로 접근하면서 반복수행
	-JAVA의 문자열.split("구분자")과 비슷 ("최,지,원".split(",") -> ["최","지","원"])
	</pre>
	
	<c:set var="device" value="컴퓨터,노트북/핸드폰,모니터/냉장고"></c:set>
	...
	<a href="list.do?cpage=1&num=2"></a>
