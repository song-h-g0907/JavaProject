...
public class ListRun {
	 ...
	 *  > 배열의 단점
	 *  	1. 배열은 크기를 지정해야한다.
	 *  	   새로운 값을 더 추가하고자 한다면 새로운 크기의 배열을 만들고 기존의 값을 전부 복사하는 코드르 작성해야한다.
	 *  
...
//		ArrayList list
		...
		list.add(new Music("healer","day6"));
		list.add(new Music("하루하루","빅뱅"));
		...
		//6. get(int index) : 해당 인덱스 위의 객체를 반환시켜주는 메소드
		Music m = (Music)list.get(0);
		System.out.println(m);
		
		//7. subList(int index1, int index2) : List -> 추출해서 새로운 List로 변환
		List sub = list.subList(0, 2); //index1부터 index2전까지
