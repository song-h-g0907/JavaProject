...
ORDER BY DEPT_CODE; -- 4 ORDER BY 무조건 마지막샐행 -> 정렬은 마지막 해주는 것이 아니면 의미X
...
/*
    ...
    -INTERSECT : AND | 교집합(두 쿼리문에 수행한 결과값에 중복된 결과값)
...
--정렬하고싶다면 ORDER BY 는 마지막에 기술한다.

--3. UNION ALL : 여러개의 쿼리결과를 무조건 다 더한다(중복 상관 X)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION ALL
...
--4. MINUS : 선생 SELECT결과에서 후행 SELECT결과를 뺀 나머지
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
MINUS
