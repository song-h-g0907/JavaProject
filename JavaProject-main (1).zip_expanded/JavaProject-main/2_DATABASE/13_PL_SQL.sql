/*
    ...
    SQL문장 내에서 변수의 정의, 조건,(IF), 반복(FOR, WHILE)등을 지원하여 SQL단점을 보안
    다수의 SQL문을 한번에 실행가능
...
--------------------------------------------------------------------------------
--1_2) 래퍼런스 타입 변수 선언 및 초기화(어떤테이블의 어떤 컬럼의 데이터타입을 참조해서 그 타입으로 지정)
...
--2. BEGIN 실행부
-- <조건문>
-- 1) IF 조건식 THEN 실행내용 END IF; (IF문을 단독으로 상요할 때)
