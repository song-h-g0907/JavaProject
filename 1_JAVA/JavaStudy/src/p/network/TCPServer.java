...
public class TCPServer {
	 ...
	 * socket
	 * - 프로세스간에 통신을 담당
	 * -input/outputSteream을 가지고 있다.(해당 스트림을 이용해서 입출력이 가능하다.)
	 * 
	 * ServerSocket
	...
	public static void main(String[] args) {
		...
		try {
			...
			//3)요청을 기다렸다 요청이 오면 -> 요청 수락 후 클라이언트와 통신할 수 있는 서버쪽 소켓 객체 생성
			while(true) {
				Socket socket = server.accept();
				System.out.println(socket.getInetAddress() + "가 연결을 요청함...");
			...
			//입력용 스트림(클라이언트로부터 전달된 값을 한줄단위 읽어들이기 위해서 입력용 스트림 사용)
